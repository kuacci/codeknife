...
时间复杂度： O(lgN) ~ O(N). 由于是二分法，理论上时间复杂度可以到达O(lgN), 但是如果出现了重复的情况，就进行单步扫描，直到找到不重复的情况，所以最坏的情况就是都为重复数，扫描一遍数组，为O(N).
空间复杂度：O(1), 没有借助额外的变量。
...
时间复杂度： O(2lgN) = O(lgN). 二分法寻找翻转点为O(lgN), 再用二分法在相应的区域找target, 为O(lgN).
空间复杂度: O(lgN)借用了线程的stack来做递归。因为用的是二分法，最坏的情况下是做了lgN次计算确定了翻转点的情况，stack的深度为lgN.
